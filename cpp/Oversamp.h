...
#define UPMEM2(i) (upmem2[i])
#define DOMEMPUT2(i) (domemoff2[oversampidx+(i)])
#define DOMEMGET2(i) (domemoff2[(oversampidx-(i))])
...
#define OVERSAMPLE2_2_INIT                              \
    memset(unit->m_domem2, 0, sizeof(float)*256);       \
    memset(unit->m_upmem2, 0, sizeof(float)*8)



struct Oversamp2 : public Unit
{
    float m_upmem[8];
    float m_domem[256];
};
...
#define OVERSAMPLE2_INIT                                \
    memset(unit->m_domem, 0, sizeof(float)*256);        \
    memset(unit->m_upmem, 0, sizeof(float)*8)


#define UPSAMPLE2                                                       \
    float *upmem = unit->m_upmem;                                       \
    float *domemoff = unit->m_domem+18;                                 \
    int oversampidx = 0;                                                \
    LOOP(inNumSamples,                                                  \
         float x;                                                       \
         UPMEM(0) = x = ZXP(in);                                        \
         DOMEMPUT(0) = UPMEM(4);                                        \
         DOMEMPUT(1) =                                                  \
         -0.00368293562290173629586442772f * (x + UPMEM(7)) +           \
         0.03564141765968901337524599171f  * (UPMEM(1) + UPMEM(6)) +    \
         -0.13979545560660305847378026556f * (UPMEM(2) + UPMEM(5)) +    \
         0.60882254392575707591106493055f * (UPMEM(3) + UPMEM(4));      \
         UPMEM(7) = UPMEM(6); UPMEM(6) = UPMEM(5); UPMEM(5) = UPMEM(4); \
         UPMEM(4) = UPMEM(3); UPMEM(3) = UPMEM(2); UPMEM(2) = UPMEM(1); \
         UPMEM(1) = UPMEM(0);                                           \
         oversampidx+=2;                                                \
...
#define DOWNSAMPLE2STD                                                  \
    oversampidx = 0;                                                    \
    LOOP(inNumSamples,                                                  \
         float outx;                                                    \
         outx =                                                         \
         0.00075825145177388689719799597f   * (DOMEMGET(0) + DOMEMGET(18))  + \
         -0.00243374824921089964735454458f  * (DOMEMGET(1) + DOMEMGET(17))  + \
         0.00547375420930718697976047338f  * (DOMEMGET(2) + DOMEMGET(16))  + \
         -0.01049035310626213884921398289f * (DOMEMGET(3) + DOMEMGET(15))  + \
         0.01840957586796767089798443351f  * (DOMEMGET(4) + DOMEMGET(14))  + \
         -0.03097452982485026512504688867f * (DOMEMGET(5) + DOMEMGET(13)) + \
         0.05253681569174622340012703603f * (DOMEMGET(6) + DOMEMGET(12)) + \
         -0.09909951460846741499999268399f * (DOMEMGET(7) + DOMEMGET(11)) + \
         0.31591830587859187629007351461f   * (DOMEMGET(8) + DOMEMGET(10)) + \
         0.5f * DOMEMGET(9);                                            \
         ZXP(out) = outx;                                               \
         oversampidx+=2;                                                \
         );                                                             \
    memmove((domemoff-18), domemoff+(inNumSamples*2-18), sizeof(float)*18)
...
struct Oversamp8_2 : public Unit
{
    float m_upmem[8];
    float m_domem[584];
    float m_upmem2[8];
...
#define OVERSAMPLE8_2_INIT                              \
    memset(unit->m_domem2, 0, sizeof(float)*584);       \
    memset(unit->m_upmem2, 0, sizeof(float)*8)



#define UPSAMPLE8_2                                                     \
    float *upmem2 = unit->m_upmem2;                                     \
    float *domemoff2 = unit->m_domem2+72;                               \
    oversampidx = 0;                                                    \
    LOOP(inNumSamples,                                                  \
         float x;                                                       \
         UPMEM2(0) = x = ZXP(in2);                                      \
         DOMEMPUT2(0) = UPMEM2(4);                                      \
         DOMEMPUT2(1)=                                                  \
         -0.0037931994f*x                                               \
         +0.0158813611f*UPMEM2(1)                                       \
         -0.0442545016f*UPMEM2(2)                                       \
         +0.1287107838f*UPMEM2(3)                                       \
         +0.9753976635f*UPMEM2(4)                                       \
         -0.0948656533f*UPMEM2(5)                                       \
         +0.0347190922f*UPMEM2(6)                                       \
         -0.0117955463f*UPMEM2(7);                                      \
         DOMEMPUT2(2)=                                                  \
         -0.0088142121f *x                                              \
         +0.0338077224f *UPMEM2(1)                                      \
         -0.0924474127f *UPMEM2(2)                                      \
         +0.2840130427f *UPMEM2(3)                                      \
         +0.8982382508f *UPMEM2(4)                                      \
         -0.1530045572f *UPMEM2(5)                                      \
         +0.0568189369f *UPMEM2(6)                                      \
         -0.0186117707f *UPMEM2(7);                                     \
         DOMEMPUT2(3)=                                                  \
         -0.0141879675f *x                                              \
         +0.0506097695f *UPMEM2(1)                                      \
         -0.1365747821f *UPMEM2(2)                                      \
         +0.4539031668f *UPMEM2(3)                                      \
         +0.7767189331f *UPMEM2(4)                                      \
         -0.1754713746f *UPMEM2(5)                                      \
         +0.0655667940f *UPMEM2(6)                                      \
         -0.0205645392f *UPMEM2(7);                                     \
         DOMEMPUT2(4)=                                                  \
         -0.0186127440f *(x+UPMEM2(7))                                  \
         +0.0624728497f *(UPMEM2(1)+UPMEM2(6))                          \
         -0.1673546124f *(UPMEM2(2)+UPMEM2(5))                          \
         +0.6234945067f *(UPMEM2(3)+UPMEM2(4));                         \
         DOMEMPUT2(5)=                                                  \
         -0.0205645392f *x                                              \
         +0.0655667940f *UPMEM2(1)                                      \
         -0.1754713746f *UPMEM2(2)                                      \
         +0.7767189331f *UPMEM2(3)                                      \
         +0.4539031668f *UPMEM2(4)                                      \
         -0.1365747821f *UPMEM2(5)                                      \
         +0.0506097695f *UPMEM2(6)                                      \
         -0.0141879675f *UPMEM2(7);                                     \
         DOMEMPUT2(6)=                                                  \
         -0.0186117707*x                                                \
         +0.0568189369f*UPMEM2(1)                                       \
         -0.1530045572f*UPMEM2(2)                                       \
         +0.8982382508f*UPMEM2(3)                                       \
         +0.2840130427f*UPMEM2(4)                                       \
         -0.0924474127f*UPMEM2(5)                                       \
         +0.0338077224f*UPMEM2(6)                                       \
         -0.0088142121f*UPMEM2(7);                                      \
         DOMEMPUT2(7)=                                                  \
         -0.0117955463f*x                                               \
         +0.0347190922f*UPMEM2(1)                                       \
         -0.0948656533f*UPMEM2(2)                                       \
         +0.9753976635f*UPMEM2(3)                                       \
         +0.1287107838f*UPMEM2(4)                                       \
         -0.0442545016f*UPMEM2(5)                                       \
         +0.0158813611f*UPMEM2(6)                                       \
         -0.0037931994f*UPMEM2(7);                                      \
         UPMEM2(7) = UPMEM2(6); UPMEM2(6) = UPMEM2(5); UPMEM2(5) = UPMEM2(4); \
         UPMEM2(4) = UPMEM2(3); UPMEM2(3) = UPMEM2(2); UPMEM2(2) = UPMEM2(1); \
         UPMEM2(1) = UPMEM2(0);                                         \
         oversampidx+=8;                                                \
         )


#define DOWNSAMPLE8_2                                                   \
    oversampidx = 0;                                                    \
    LOOP(inNumSamples,                                                  \
         ZXP(out2) = 0.00248839632f * (DOMEMGET(0) + DOMEMGET(72)) +    \
         0.00258871550f * (DOMEMGET(1) + DOMEMGET(71)) +                \
         0.00221903196f * (DOMEMGET(2) + DOMEMGET(70)) +                \
         0.00133875226f * (DOMEMGET(3) + DOMEMGET(69)) +                \
         -0.00164433851f * (DOMEMGET(5) + DOMEMGET(67)) +               \
    ...
    memmove((domemoff2-72), domemoff2+(inNumSamples*8-72), sizeof(float)*72)
